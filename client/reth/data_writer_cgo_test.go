...
func TestWriteEOAsRoundtrip(t *testing.T) {
	...
	// v2: PlainAccountState must be empty (HashedAccounts is the canonical
	// state). This is the regression catcher for "future commit reintroduces
	// a PlainAccountState write" — the cross-client invariant test wouldn't
	// notice because state-root computation reads HashedAccounts only.
	if err := envs.Mdbx.View(func(txn *mdbx.Txn) error {
		...
		if err == nil {
			t.Errorf("PlainAccountState non-empty on v2 datadir — Plain* writes have been re-introduced")
		}
		return nil
	}); err != nil {
		t.Errorf("verify PlainAccountState empty: %v", err)
	}

	// Read back HashedAccounts — decode each row and verify nonce + balance.
	if err := envs.Mdbx.View(func(txn *mdbx.Txn) error {
	...
	// Read back AccountsHistory from the RocksDB CF (v2 routing). Flush
	// memtables first since the batch was written WAL-disabled.
	...
	for _, acc := range accounts {
		...
		if err != nil {
			t.Errorf("RocksDB AccountsHistory %s: %v", acc.Address.Hex(), err)
			continue
		}
		data := val.Data()
		list, _ := iReth.DecodeIntegerList(data)
		val.Free()
