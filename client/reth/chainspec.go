...
// writeChainSpec writes a Reth-compatible chainspec JSON to outPath with an
// EMPTY alloc. The state is imported via `reth init-state --without-evm
// --header` using our JSONL dump + pre-built genesis header; we only need
// the chainspec for chain config (chainId + hardfork activations) and any
// user-supplied top-level genesis fields (timestamp, gasLimit, baseFeePerGas).
//
// `chainID` overrides `config.chainId` when non-zero. When genesisPath is
// non-empty, every top-level field from that file EXCEPT alloc is copied
// in; alloc is always zeroed because it's redundant (the dump is
// authoritative) and copying it in would be hundreds of GB at the target
// scale.
