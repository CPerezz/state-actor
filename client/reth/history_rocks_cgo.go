...
// column families. v2 routes both tables to RocksDB unconditionally per
// EitherReader::new_accounts_history / new_storages_history
// (either_writer.rs:699-759); v1's MDBX tables of the same name receive no
// reads in v2 mode.
//
// Bulk path: durability is owed at Envs.Close (which FlushCFs the underlying
// memtables to SST files). Per-flush we DisableWAL so the per-row append
// doesn't pay an fsync.
//
// The sink is NOT goroutine-safe; producers must serialise puts through one
// sink, or use one sink per goroutine and Flush each before merging results.
...
// EncodeIntegerList([blockNum]) into the AccountsHistory column family. Key
// + value shapes are byte-identical to the v1 MDBX AccountsHistory rows;
