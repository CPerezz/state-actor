...
//   - --fork=osaka (geth's MaxForkForClient ceiling).
//   - 60M gas limit (mainnet-current).
//   - Boots geth in `--dev --dev.period=1` mode: PoA dev chain, 1s
//     blocks, self-contained CL emulation. No engine-API mock needed —
//     geth's --dev mode handles consensus internally.
...
func TestE2ESuite(t *testing.T) {
	...
	// Pin --fork=osaka (geth's MaxForkForClient ceiling). 60M gas limit
	// matches mainnet-current (post-Pectra) target.
